...
Load_os_flags()
{
	...
	os_flags[10].flag = IFF_MASTER;
	os_flags[10].comment = "master of a load balancer";
	os_flags[11].flag = IFF_SLAVE;
	os_flags[11].comment = "slave of a load balancer";
	os_flags[12].flag = IFF_MULTICAST;
	os_flags[12].comment = "supports multicast";
	os_flags[13].flag = IFF_PORTSEL;
	os_flags[13].comment = "can set media type";
	os_flags[14].flag = IFF_AUTOMEDIA;
	os_flags[14].comment = "auto media select active";
	os_flags[15].flag = IFF_DYNAMIC;
	os_flags[15].comment = "dialup device with changing addresses";
